            ...
            if (millis() - signalTimer >= signalTiming)
            {
                if (lineStatus == 1) // посылка вызова
                {
                    ...
                    else
                    {
                        ...
                        signalTiming = 3900;           // 4 секунды на паузу (без тех 100 мс дилэя)
                        toneSignalId = 0;

                        delay(100);                                         // без этой задержки код фиксирует просадки напряжения линии в момент смены коммутации
